...
public interface ConversationLogRepository extends CrudRepository<ConversationLog, Long> {
    ...
    @Modifying
    @Transactional
    @Query("UPDATE ConversationLog h SET h.mentorScore = :mentorScore where h = :conversationLog")
    Integer updateMentorScore(@Param("mentorScore") Integer mentorScore, @Param("conversationLog") Long conversationLogId);
    @Modifying
    @Transactional
    @Query("UPDATE ConversationLog h SET h.menteeScore = :menteeScore where h = :conversationLog")
